...
import { adminUpdateItem } from "@/api/menu";
...
export function EditItemModal({ open, onOpenChange, onSuccess, categories, item }: EditItemModalProps) {
  ...
  const onSubmit = async (values: z.infer<typeof createMenuItemSchema>) => {
    ...
    try {
      const res = await adminUpdateItem({ data: { ...values, id: item.id } });
      if (res.success) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[525px] border-white/10 bg-black/95 text-white backdrop-blur-xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-gold font-display text-xl">Edit Menu Item</DialogTitle>
        </DialogHeader>
